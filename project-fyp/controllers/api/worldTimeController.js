...
module.exports.getTime = async (req, res) => {
  try {
    ...
    if (!city && !country && !timezone) {
      return res.status(400).json({ message: "city, country, or timezone parameter required" });
    }
    ...
    if (!selectedTimezone) {
      ...
      if (!selectedTimezone) {
        for (const [key, value] of Object.entries(locationToTimezone)) {
          if (location.includes(key) || key.includes(location.split(/[,\s]+/)[0])) {
    ...
    if (!data || !data.datetime) {
      return res.status(404).json({ message: "Time data not found for this location" });
    }
    ...
    if (e.code === 'ECONNRESET' || e.code === 'ETIMEDOUT' || e.code === 'ECONNREFUSED') {
      ...
      if (!selectedTimezone) {
        ...
        selectedTimezone = locationToTimezone[loc] || locationToTimezone[firstWord] || 'UTC';
      }
      ...
      return res.json({
        ...
        time: `${localTime.getHours()}:${String(localTime.getMinutes()).padStart(2, '0')}`,
