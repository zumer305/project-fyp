...
function validateEJSFile(filePath) {
  ...
  lines.forEach((line, index) => {
    ...
    ejsTags.forEach(tag => {
      ...
      if (tag.includes('else') && !tag.trim().startsWith('<%') && braceStack.length === 0) {
        errors.push({
          line: lineNum,
  ...
  if (braceStack.length > 0) {
    errors.push({
      line: 'Multiple',
      message: `${braceStack.length} unclosed brace(s). Opened at lines: ${braceStack.slice(0, 5).join(', ')}${braceStack.length > 5 ? '...' : ''}`,
      tag: ''
    });
  }
  ...
  if (errors.length === 0) {
    ...
    console.log(`📊 Balanced braces: All opening braces have matching closing braces`);
    return true;
  } else {
