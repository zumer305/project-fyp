...
router.get("/", wrapAsync(async (req, res) => {
  try {
    ...
    const faresData = await fetchRealTimeFaresNoKey(startLat, startLng, destination);
...
async function fetchRealTimeFaresNoKey(startLat, startLng, destination) {
  try {
    ...
    const distanceKm = await getDistanceFromOSRM(startLat, startLng, endLat, endLng);
    const transportFares = await fetchFromTransportAPI(startLat, startLng, distanceKm);
...
async function fetchFromTeleportAPI(lat, lng) {
  try {
    ...
    if (cityResponse.data && cityResponse.data._embedded && cityResponse.data._embedded['location:nearest-urban-areas']) {
      const urbanArea = cityResponse.data._embedded['location:nearest-urban-areas'][0];
      const urbanAreaHref = urbanArea._links['location:nearest-urban-area'].href;
      ...
      if (detailsResponse.data && detailsResponse.data._links['ua:details']) {
        ...
        if (costResponse.data && costResponse.data.categories) {
          const transportData = costResponse.data.categories.find(cat => 
            cat.id === 'TRAVEL-CONNECTIVITY' || cat.id === 'COMMUTE'
          );
...
function parseTeleportData(data) {
  const taxiCost = data.find(d => d.id === 'TAXI-COST-PER-MILE' || d.id === 'TAXI');
...
async function getTrafficMultiplier(lat, lng) {
  try {
    ...
    const response = await axios.get('https://worldtimeapi.org/api/timezone/Asia/Tashkent', {
      timeout: 3000
    });
    ...
    if (response.data && response.data.datetime) {
      ...
      console.log(`✅ WorldTimeAPI - Current hour: ${hour}, Peak: ${isPeakHour}`);
      return multiplier;
    }
  } catch (error) {
...
function generateRealTimeFares(distanceKm, trafficMultiplier) {
  ...
  const vehicles = [
    ...
    },
    {
      ...
      imageUrl: "https://hips.hearstapps.com/hmg-prod/images/2022-chevrolet-spark-mmp-1-1638552174.jpg"
    },
    {
      name: "Daewoo Nexia",
      type: "Budget",
