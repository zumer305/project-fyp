...
async function testPackageGeneration(budgetUSD) {
  ...
  const midPkgs1 = packages1.filter(p => p.totalEstimateUSD >= 500 && p.totalEstimateUSD < 1500);
  ...
  packages1.forEach((pkg, idx) => {
    ...
    console.log(`  ${idx + 1}. ${pkg.name} - $${pkg.totalEstimateUSD} USD (${pkg.duration}) ${withinBudget}`);
  });
  ...
  const midPkgs2 = packages2.filter(p => p.totalEstimateUSD >= 500 && p.totalEstimateUSD < 1500);
  ...
  packages2.forEach((pkg, idx) => {
    const withinBudget = pkg.totalEstimateUSD <= testBudget ? '✓ Within' : '✗ Above';
    console.log(`  ${idx + 1}. ${pkg.name} - $${pkg.totalEstimateUSD} USD (${pkg.duration}) ${withinBudget} budget`);
    console.log(`     Breakdown: Hotel $${pkg.breakdownUSD.hotel}, Food $${pkg.breakdownUSD.food}, Transport $${pkg.breakdownUSD.transport}`);
  });
  ...
  console.log(`Unique prices: ${uniquePrices.size} out of ${packages2.length} packages`);
  ...
  if (allMatch) {
    console.log('✓ SUCCESS: totalEstimateUSD matches priceUSD for all packages');
  } else {
    console.log('✗ WARNING: Some packages have mismatched totalEstimateUSD and priceUSD');
  }
  ...
  const allWithinBudget = packages3.every(p => p.totalEstimateUSD <= lowBudget);
  ...
  if (allWithinBudget) {
    ...
    console.log('✓ No packages within budget, showing closest above (expected behavior)');
  } else {
    const withinCount = packages3.filter(p => p.totalEstimateUSD <= lowBudget).length;
    const aboveCount = packages3.filter(p => p.totalEstimateUSD > lowBudget).length;
    console.log(`✓ Mixed: ${withinCount} within budget, ${aboveCount} above (filling to 4 total)`);
  }
  
  packages3.forEach((pkg, idx) => {
    const status = pkg.totalEstimateUSD <= lowBudget ? 'Within' : 'Above';
    console.log(`  ${idx + 1}. ${pkg.name} - $${pkg.totalEstimateUSD} USD (${status} budget)`);
  });
  ...
  packages4.forEach((pkg, idx) => {
    ...
    console.log(`  ${idx + 1}. ${pkg.name} - $${pkg.totalEstimateUSD} USD ${status}`);
  });
