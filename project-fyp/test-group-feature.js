...
async function test() {
  ...
  try {
    ...
    const groupDetailsRes = await axios.get(
    ...
    console.log(
      "   Group now has",
      joinRes.data.group.members.length,
      "members"
    );

    // Test 11: User 2 Send Message
    console.log("\n💬 Test 11: User 2 sending a message...");
    const message2Res = await axios.post(
      `${BASE_URL}/api/groups/${groupId}/messages`,
      {
    ...
    const allMessagesRes = await axios.get(
      `${BASE_URL}/api/groups/${groupId}/messages`,
      {
        headers: { Authorization: `Bearer ${authToken}` },
      }
    ...
    const leaveRes = await axios.post(
      `${BASE_URL}/api/groups/${groupId}/leave`,
      {},
      {
        headers: { Authorization: `Bearer ${authToken2}` },
      }
